...
import type { EVResult } from "@/lib/llm-content";
...
function getRecommendations(quiz: QuizState, evs: EVResult[]): EVResult[] {
  return [...evs]
...
export default function EVQuiz({ initialEVs }: { initialEVs: EVResult[] }) {
  ...
  if (showResults) {
    const results = getRecommendations(quiz, initialEVs);
    return (
